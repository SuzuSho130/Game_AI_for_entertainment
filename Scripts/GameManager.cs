...
public class GameManager : MonoBehaviour {
	...
	PlayerController _player_conttoller;
	EnemyController _enemy_controller;
	AI _player2_AI;
	EvalutionAI _player1_AI;

	private const int Group = 4;
	private const int GeneLength = 40;      // 遺伝子長
	...
	void Start() {
		_player_conttoller = Player1.GetComponent<PlayerController> ();
		_enemy_controller = Player2.GetComponent<EnemyController> ();
		_player2_AI = Player2.GetComponent<AI> ();
		_player1_AI = Player1.GetComponent<EvalutionAI> ();
//		Run ();
	}
	...
	public void Run()
	{
		...
		while (true)
		{
			...
			foreach (Data data in currentPopulation) {
				_player2_AI.SetStatusValue (data.GetMmbers ());
//				for (int i = 0; i < 3600; i++) {
//					_player_conttoller.OneAction ();
//					_enemy_controller.OneAction ();
//				}
//				while (battle) {
//				}
				Player1.transform.localPosition = new Vector3 (-3.45f, -1.71f, 0.0f);
				Player2.transform.localPosition = new Vector3 (2.91f, -1.71f, 0.0f);
				data.SetEvalution (_enemy_controller.GetHP());
			}
